import { query, where, collection, addDoc ,getDocs, doc, updateDoc,deleteDoc,getDoc } from "firebase/firestore";
...
export const checkpaymentDetailsUniqueness = async (accnum) => {
  ...
  export const isMerchantIdExist = async () => {
    try {
      const snap = await getDoc(doc(config.db, "PayPalMerchantInfo", getRestaurantId()));
  
      if (snap.exists()) {
        const data = snap.data();
        const isMerchantIdInPayPal = data.hasOwnProperty("merchantIdInPayPal");
        return isMerchantIdInPayPal;
      } else {
        return false;
      }
    } catch (e) {
