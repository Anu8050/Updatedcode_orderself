...
export const newPerformBulkCSVUploadOperation = async (menuData) => {
    try {
        ...
        return Promise.all(promises).then(async (foodCats) => {
            ...
            menuData.map(async (csvRow) => {
                return await checkFoodNameUniqueness(csvRow.foodName.trim()).then(async (foodNameExists) => {
                    if (!foodNameExists) {
                        var foodCategoryId = foodCats[0].filter(c => c.foodCategory === csvRow.foodCategory.trim())[0].id;
...
export const checkFoodCategoryNewUniqueness = async (categoryName, docId) => {
    try {
        ...
        foodCat = foodCat.filter(word => word.foodCategory.toLowerCase() == categoryName.toLowerCase())
        return Promise.resolve((foodCat.length > 0) ? true : false);
    } catch (error) {
        console.error("Error adding document: ", error);
        return "error";
...
export const updatePayPalMerchantInfo = async (
    restaurantId,
    newObj
  ) => {
    try {

      const docRef = doc(config.db, "PayPalMerchantInfo", getRestaurantId());
