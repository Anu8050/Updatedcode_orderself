...
export default function RestaurantOrders(props) {
  ...
  const [tableAvilable, setTableAvilable] =React.useState(false);
  ...
  useEffect(() => {
    ...
    const tableAvailability = orderService.checkTableActiveStatus(params.restaurantId,params.tableNumber)
    tableAvailability.then((res)=>{
      if(res)
      {
      setTableAvilable(true)
      }
    })
  }
  }, []);
  ...
  const saveCustomerInfo = () => {
    ...
    localStorage.clear();
    if(tableAvilable == false)
    {
      NotificationManager.warning("Table does not exist, Please contact the waiter.");
    }
    else{
