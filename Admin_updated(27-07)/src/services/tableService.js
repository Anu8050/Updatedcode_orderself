...
export const getTableDetails = async (restaurantId, porterId = "") => {
    try {
        var q = undefined;
        if (porterId === ""){
            q = query(collection(config.db, constants.TABLE_INFO), where("restaurantId", "==", restaurantId),where("active", "==", true));
       }
       else if (porterId !== ""){
...
export const disableTable = async (tableNumber) => {
    try {
      ...
      const updatePromises = querySnapshot.docs.map(async (docSnapshot) => {
        const docRef = doc(config.db, constants.TABLE_INFO, docSnapshot.id);
        await updateDoc(docRef, {
          active: false,
        });
      });
      await Promise.all(updatePromises);  
      console.log(`Table  active status updated.`);
    } catch (err) {
      console.error(err);
    }
  };

  export const disableTablesBulk = async (tableNumbers) => {
    try {
      for (const tableNumber of tableNumbers) {
        await disableTable(tableNumber);
      }
  ...
  export const enableTable = async (tableNumber) => {
    try {
      const restaurantId = localStorage.getItem("restaurant-id");
      const q = query(
        collection(config.db, constants.TABLE_INFO),
  ...
  export const enableTablesBulk = async (tableNumbers) => {
    try {
      for (const tableNumber of tableNumbers) {
        await enableTable(tableNumber);
      }
      console.log("All tables updated.");
    } catch (err) {
      console.error(err);
