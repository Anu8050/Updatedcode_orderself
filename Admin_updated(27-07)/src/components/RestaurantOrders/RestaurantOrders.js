...
const CustomStatusCellRenderer = ({ params }) => {
  ...
  const tableNumber = params.row.tableId;
  ...
  const handleStatusChange = async (event) => {
    ...
    if (["Completed", "Cancelled"].includes(status)) {
      ...
      setConfirmationMessage(
        status === "Completed"
          ? <span>Before you complete the order, please be sure the payment is received by the guest. If not, you will encounter a loss in your bill. Are you still sure, to change the status to completed?</span>
          : <span>You are about to cancel the order placed in <b>table number {tableNumber}.</b> Are you sure you want to cancel the order?</span>
      );
      setConfirmDialogOpen(true);
    } else {
      setSelectedStatus(status);
      try {
        const response = await updateOrderStatus(rowId, status);
        console.log("🚀 ~ file: RestaurantOrders.js:46 ~ handleStatusChange ~ response:",response );
      } catch (error) {
      console.log(
  ...
  const handleDialogConfirm = async () => {
    ...
    try {
      ...
      console.log("🚀 ~ file: RestaurantOrders.js:47 ~ handleStatusChange ~ error:",error);
    }
  ...
  return (
    ...
    <Dialog open={confirmDialogOpen} onClose={handleConfirmDialogClose}>
        <DialogTitle sx={{fontWeight:"bold" , fontSize:"1.875rem"}}>Are you sure ?</DialogTitle>
        <DialogContent style={{ color: "gray" }}>{confirmationMessage}</DialogContent>
        <DialogActions>
          <Button onClick={handleConfirmDialogClose} variant="outlined" startIcon={<CloseIcon/>}>
            Cancel
          </Button>
          <Button onClick={handleDialogConfirm} variant= "contained" startIcon={<CheckIcon/>} autoFocus>
            Confirm
          </Button>
        </DialogActions>
      </Dialog>
    ...
    <>
      <Box
        ...
        <Grid
          ...
          marginBottom={0.5}
        >
          <Grid item xs={3}  md={7.5} xl={10}  className="breadcrumb">
            <span>Order Status</span>
          </Grid>
          {porterId && (
            <>
            <Grid item xs={2.9} md={2} xl={.9} style={{ textAlign:"center" }}>
              <Switch checked={myOrders} onChange={handleMyOrders}/>
              <label>My Orders</label>
            </Grid>
            <Grid item xs={.2} md={.2} xl={.2} style={{display: "flex",justifyContent: "center",alignItems: "center"}}>
              <Divider orientation="vertical" sx={{bgcolor:"gray", width:"2px"}} />
            </Grid>
            <Grid item xs={2.9} md={2} xl={.9}>
